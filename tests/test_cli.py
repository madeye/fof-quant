...
def test_backtest_run_command(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(
        app,
        ["backtest", "run", "--config", str(config_path)],
    )

    assert result.exit_code == 0
    assert (report_dir / "backtest.json").exists()


def test_report_build_command(tmp_path: Path) -> None:
    example_config = Path("configs/example.yaml").read_text(encoding="utf-8")
    config_path = tmp_path / "config.yaml"
    report_dir = tmp_path / "reports"
    config_path.write_text(
    ...
    result = CliRunner().invoke(
        app,
        ["report", "build", "--config", str(config_path)],
    )
